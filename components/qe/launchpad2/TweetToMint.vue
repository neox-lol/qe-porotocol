...
const { loadMintedData, loadTokenData, loadTokenOnChainData } = $(ixoQE721Store());
...
const text = $computed(
  () => `Follow @${tokenMeta.twitter}, RT and LIKE via ${shareLink} to get your ${tokenMeta.freeMintAmountPerUser} $QE1155${mintJson}`
);
const { needMoreGas } = $(taskStore());
const twitterShareBtnLink = $computed(() => {
  return `https://twitter.com/intent/tweet?text=${text}&url=${tokenMeta.retweetStatusLink}&hashtags=${tokenMeta.retweetTags}`;
});
...
const doSaveRetweetLink = async () => {
  ...
  const { data } = await getSigFromServer("/api/ixo/QE1155Retweet", {
    tokenId,
    link,
    network,
  ...
  const loading = addLoading(`Start mint ${tokenMeta.freeMintAmountPerUser} $QE1155 FT, be patient!`);
  let transactionHash = "";
  try {
    const payment = getContractAddress("AIEntropy", network);
    const payAmount = 0;
    ...
    const { tx } = await writeContract(
      ...
      to,
      tokenId,
      tokenMeta.freeMintAmountPerUser,
      payment,
      payAmount,
      nonce,
      sig
  ...
  const { data: mintedData } = await usePost("/api/ixo/QE1155RetweetMinted", { transactionHash, tokenId, network });
  if (mintedData?.error) {
    isLoading = false;
    return alertError(mintedData.error.message, false, loading);
  }
  alertSuccess(
    `Mint  ${tokenMeta.freeMintAmountPerUser} $QE1155 FT Succeed!`,
    async () => {
      await loadMintedData(tokenId, true);
      await loadTokenData(tokenId, true);
      await loadTokenOnChainData(tokenId, true);
      isLoading = false;
      isShow = false;
    },
    loading
    ...
    <BsDialogDefault :show="isShow" :no-padding="true" @close="isShow = false">
      <div class="bg-lime-600 max-w-2xl py-6 px-3 shadow-2xl relative isolate overflow-hidden sm:rounded-3xl sm:px-12 xl:py-8">
        <h2 class="font-bold mx-auto my-10 text-center text-white text-xl tracking-tight max-w-2xl sm:text-2xl">
          ...
          <br />
          <br />
          <span text-rainbow uppercase text-6xl animate-pulse> {{ tokenMeta.freeMintAmountPerUser }} $QE1155 FT </span>
        </h2>
        <div flex-cc>
